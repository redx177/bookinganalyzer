...
Die folgenden Methoden geben eine Übersicht über die gängigsten Techniken im Data Mining wider\footcite{top_10_algorithms}. Beschrieben werden Problemstellungen, aus welchen im Konzept\todo{cref} dann passende Algorithmen ausgewählt werden.
...
\end{lstlisting}
Zu Beginn werden alle Punkte als ''unvisited'' makriert. Anschliessend wird ein zufälliger Punkt $p$ gewählt welcher als ''visited'' markiert wird. Auf Zeile 4 wird überprüft ob die \textit{neighborhood density} von $p$ den mindestwert $minpts$ erfüllt. Wenn ja wird für diesen Punkt ein Cluster erstellt und all seine Nachbaren in ''candidates'' gespeichert. Die Zeilen 7-10 überprüfen ob die ''candidates'' noch ''unvisited'' sind. Wenn ja wird der Punk als ''visited'' markiert und all seine Nachbaren der Liste ''candidates'' angehängt.
