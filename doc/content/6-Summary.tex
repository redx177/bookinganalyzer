...
Als Nächstes wird der Proof of Concept der Interhome vorgestellt. Sie sind interessiert an dem Produkt und wollen es gerne ausprobieren. Der Einkauf und das Marketing sollen deshalb das Programm ausgiebig einsetzen, wodurch hoffentlich wertvolles Feedback entsteht. Es ist zu erwarten, dass sie weitere Attribute bei der Analyse benötigen. Wie zum Beispiel die Anzahl der reisenden Kinder.

Im \cref{sec:recherche:datenvorbereitung:diskretierung} wurden die Distanzen und der Wochenpreis in fixe Wertebereiche diskretiert. Andere Möglichkeiten wurden erwähnt, jedoch nicht weiter untersucht. Dies ist kein Problem solange verschiedene Resultate vom Apriori Algorithmus verglichen werden. Wenn zum Beispiel analysiert wird, wie viele Buchungen in Frankreich im Gegensatz zu der Schweiz das Attribute "`nahe am Skilift"' besitzen, kann das Resultat miteinander verglichen werden. Beide Analysen verwenden die selbe Diskretierungswerte. Wenn hingegen eine Analyse nur für Frankreich gemacht wird, sieht man dass 19.45\% aller Buchungen nahe am Skilift liegen. Hier hat die Diskretierung einen Einfluss auf das Resultat, denn es macht einen Unterschied, ob der Wertebereich von 0-499, oder 0-1'000 Meter gewählt wird. Es gibt verschiedene Techniken für das Diskretieren, wie zum Beispiel "`equal-width"', oder "`equal-frequency"'\footcite{data_mining_concepts_and_techniques}. Dies könnte weiter untersucht werden.

Die Diskretierung ermöglicht es die numerischen Werte in der Apriori Analyse zu verwenden. Die Distanz von 199, oder 200 Metern sind beide "`close"', jedoch bei 499 und 500 Metern ist dies nicht der Fall. Da ist der erste Wert "`close"', der Zweite jedoch nicht. Dieses Verhalten ist nicht hilfreich beim Clustering. Deshalb empfiehlt es sich die k-prototype-, oder DBSCAN Algorithmen nochmals ohne die Diskretierung einzusetzen. Wohlmöglich werden dadurch bessere Resultate produziert. 
...
Das Programm kann für die Analyse mittels dem Apriori Algorithmus von der Interhome verwendet werden. Für den produktiven Einsatz sollte jedoch die Beschreibung der möglichen Einstellungen im Programm noch verständlicher erläutert werden. 