\chapter{Konzept}
\label{sec:konzept}
Im Konzept wird beschrieben, wie die Problemstellung, welch im \cref{sec:einleitung:ziel} aufgezeigt wurde, gelöst werden kann. Dazu werden die einzusetzenden Algorithmen evaluiert und aufgezeigt, wie diese angewendet werden. Anschliessend folgt eine Beschreibung der Daten, welche von den Algorithmen verwenden, sowie deren fünfstufige  Vorbereitung. Im Abschnitt Ablauf werden die zwei Schritte gezeigt, welche bei jeder Analyse durchgeführt werden. Zum Schluss werden noch Testfälle definiert, welche die korrekte Funktionsweise der Analyse sicherstellen.
...
Wie im \cref{sec:recherche:dataminingtechniken:disziplinen:classification} \nameref{sec:recherche:dataminingtechniken:disziplinen:classification} beschrieben ist das Ziel der Classification die Zuweisung einer Instanz zu einer Klasse. Die hätte sicherlich einige interessante Einsatzgebiete bei der Interhome. Zum Beispiel könnte die Liquidität von Kunden unterteilt werden in zwei Gruppen "`zahlungsfähig"' und "`zahlungsunfähig. Oder das Suchverhalten der Kunden und ob es wahrscheinlich ist ob er eine Buchung abschliesst oder nicht.
