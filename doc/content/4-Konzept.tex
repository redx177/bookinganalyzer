\chapter{Konzept}
\label{sec:konzept}
Im Konzept wird beschrieben wie die Problemstellung aus dem \cref{sec:einleitung:ziel} \nameref{sec:einleitung:ziel} gelöst werden kann. Dazu werden die einzusetzenden Algorithmen evaluiert und aufgezeigt, wie diese angewendet werden. Anschliessend folgt eine Beschreibung der Daten, welche von den Algorithmen verwendet werden, sowie deren fünfstufige Vorbereitung. Im Abschnitt Ablauf werden die zwei Schritte gezeigt, welche bei jeder Analyse durchgeführt werden. Zum Schluss werden noch Testfälle definiert, welche die korrekte Funktionsweise der Analyse sicherstellen.
...
Der k-prototypes benötigt die Anzahl an Clustern $k$ die generiert werden sollen. Zu Beginn einer Analyse ist jedoch noch unklar wie dieser Parameter gewählt werden soll. Mithilfe der Elbow-Methode kann der optimale Wert für $k$ ermittelt werden. Dazu wird der Algorithmus mit $k=1$ durchgeführt und der Wert nach jeder Durchlauf um jeweils 1 erhöht. Der Gesamtfehler des Clusterings (total within-cluster variation) wird schlussendlich über alle Werte für $k$ in ein Koordinatensystem eingetragen. Dies ergibt eine Kurve wie sie in \cref{fig:konzept:parameterauswahl:kprototypes} aufgezeigt wird.
...
Die Datenbasis beinhaltet bestätigte und stornierte Buchungen. Der Status wird in der Zeile C BOOKSTA gespeichert und besitzt den Wert "`OK"' wenn der Kunde die Rechnung bezahlt sowie die Reise angetreten ist, sonst beinhaltet das Feld die Buchstaben "`XX"'. Gründe für eine Stornierung gibt es viele. Zum Beispiel wenn er falsch gebucht hat, die Rechnung nicht bezahlte, die Reise nicht angetreten werden konnte, etc. 

Der Grund für die Stornierung ist in der Datenbasis nicht vorhanden. Deshalb besteht die Gefahr, dass wenn stornierte Buchungen verwendet werden die Analyse verfälscht wird. Zum Beispiel wenn ein Kunde aus versehen eine Objekt mit der Dauer von drei Monate anstelle von 3 Tage gebucht hat. Deshalb werden alle Instanzen mit dem Wert "`XX"' aus der Datenbasis entfernt.
...
\begin{itemize}
\item Region und Ort des Objektes. In der Datenbasis ist nur das Land vorhanden.
...
TC10 Überprüft ob 2 Cluster gefunden werden. Der erste besitzt 5 Buchungen, der zweite deren 4. Ziel des Testfalles ist es zu überprüfen, ob die Clusters korrekt erstellt werden und ob der Apriori-Algorithmus richtig auf die Gruppen angewendet wird.
...
TC 11 Überprüft ob 1 Cluster gefunden wird mit 9 Buchungen und 1 Noise Punkt. Bei TC10 wurden zwei ähnlich grosse Cluster generiert. Hier wird überprüft ob die Clustering- und der Apriori-Algorithmus korrekt funktioniert wenn die Gruppen unterschiedliche Grössen haben. Der kleine Cluster hat zusätzlich so wenig Objekte, dass der Apriori keine häufigen Attribute auffindet.
...
Der DBSCAN sollte die selben Cluster wie der k-prototype finden, jedoch wird die Buchung mit der ID 2 als Noise aussortieren. Dies ist die einzige Instanz, welche zu allen anderen Buchungen minimal zwei unterschiedliche kategorische Attribute besitzt. Jedes kategorische Attribut welches unterschiedlich ist erhöht die Distanz um 1. Sprich die Buchung hat zu allen anderen Instanzen minimal den Abstand von 2. Da $\epsilon$ (Radius) auf 2 gesetzt ist wird ID 2 als Noise behandelt (siehe Berechnung der \textit{neighborhood density} in \cref{sec:recherche:algorithmen:dbscan} \nameref{sec:recherche:algorithmen:dbscan}).
